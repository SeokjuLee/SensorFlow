...
parser.add_argument("--dataset-format", type=str, default='kitti_raw', choices=["euroc_mav", "kitti_raw", "cityscapes", "kitti_odom"])
...
def dump_example(args, scene):
    ...
    for scene_data in scene_list:
        ...
        zoom_x = args.width/scene_data['w']
        zoom_y = args.height/scene_data['h']
        K_new = np.vstack([scene_data['K_r'][0]*zoom_x, scene_data['K_r'][1]*zoom_y, scene_data['K_r'][2]])

        # dump_cam_file = dump_dir/'cam.txt'
        # np.savetxt(dump_cam_file, K_new)

        poses_file = dump_dir/'poses.txt'
        poses = []
        # body_poses_file = dump_dir/'body_poses.txt'
        # body_poses = []
        ...
        for sample in data_loader.get_scene_imgs(scene_data):
            ...
            poses.append(sample["pose"][:3].tolist())
            # body_poses.append(sample["body_pose"].tolist())
        if len(poses) != 0:
            np.savetxt(poses_file, np.array(poses).reshape(-1, 12), fmt='%.6e')
        # if len(body_poses) != 0:
        #     np.savetxt(body_poses_file, np.array(body_poses).reshape(-1, 7), fmt='%.6e')
...
def main():
    ...
    if args.dataset_format == 'euroc_mav':
        from euroc_mav_loader import EurocMavLoader
        data_loader = EurocMavLoader(args.dataset_dir,
