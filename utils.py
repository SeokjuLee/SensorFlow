...
# def save_checkpoint(epoch, save_path, dispnet_state, exp_pose_state, is_best, filename='checkpoint.pth.tar'):
...
def save_checkpoint(epoch, save_path, sfnet_state, is_best, filename='checkpoint.pth.tar'):
    file_prefixes = ['sfnet']
    states = [sfnet_state]
    for (prefix, state) in zip(file_prefixes, states):
        torch.save(state, save_path/'{}_{}'.format(prefix, filename))
